...
function PropertyMap({ countryCode }: { countryCode: string }) {
  const defaultLocation = [46.9929793, 6.9319325] as [number, number]
  const location = getCountryByCode(countryCode)?.location as [number, number]

  return (
    <div className="mt-4">
      <div className="mb-4 ">
        <Title text="Where you will be staying" />
        <CountryFlagAndName code={countryCode} />
      </div>
      <MapContainer
